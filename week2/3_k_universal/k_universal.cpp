...
string get_bin_string(const int num, const int num_bits) {
    // Function to get a string with the binary representation of a given number
    string bin_string(num_bits, ' ');
...
vector<string> get_k_mers(const int k) {
    // Function to generate all possible unique binary k-mers of length k.
    // These k-mers represent all possible substrings of length k over a binary alphabet.
    int num_vertices = 1 << k;
    vector<string> k_mers(num_vertices);

    for (int vertex = 0; vertex < (1 << k); ++vertex) {
        k_mers[vertex] = get_bin_string(vertex, k);
    }
...
vector<vector<int>> bruijn_graph_k_univ(vector<string>& k_mers, unordered_map<string, int>& k_minus_1_to_vertex, int& num_edges, const int k) {
...
int main() {
    ...
    vector<string> k_mers = get_k_mers(k);
    ...
    int num_edges;
    vector<vector<int>> graph = bruijn_graph_k_univ(k_mers, k_minus_1_to_vertex, num_edges, k);

    // Find an Eulerian cycle in the constructed de Bruijn graph
    list<int> cycle = get_eulerian_cycle(graph, num_edges);

    // Reconstruct and print the k-universal circular binary string
